...
auto run() -> int {
  ...
  // capacity_bytes is fixed for the mapping's lifetime (see design.md §2.6);
  ...
  const auto path = std::filesystem::temp_directory_path() / "plf_skiplist_basic_usage_example.dat";
  const size_t capacity_bytes = sizeof(plf_skiplist::DurableNode<int>) * 1002;

  {
    plf_skiplist::PSkipList<int> list(path, capacity_bytes);
  ...
  // written after it.
  plf_skiplist::PSkipList<int> recovered(path, capacity_bytes);
...
auto main() -> int {
  try {
    ...
    std::fprintf(stderr, "plf_skiplist_basic_usage: %s\n", e.what());
    return 1;
  }
}
