...
class PSkipList {
 public:
  ...
  explicit PSkipList(const std::filesystem::path &path, size_t capacity_bytes)
    ...
    if (capacity_slots_ < 2) {
      throw std::invalid_argument("plf_skiplist: capacity_bytes too small to hold head/tail sentinels");
    }
    if (capacity_slots_ > kMaxTaggedOffset) {
      throw std::invalid_argument("plf_skiplist: capacity_bytes exceeds the maximum representable node count");
    }
    if (file_.reused()) {
      recover();
    } else {
 ...
 private:
  ...
  // first untrusted node. See design.md 2.2/4.1 for the shadow-revert rule.
  void recover() {
    ...
    // manifest write (see design.md 7). A separate pass since recovery's single-
  ...
  // Cascading top-down walk comparing chunks by primary_key (see design.md 2.4.1):
